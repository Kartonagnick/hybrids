...
class Tree
  ...
  public sub show(deep, title, v)
    ...
    if mSHOW_TYPE then
      echo deep + deep_start, "<Title> " & title
    else
      echo deep + deep_start, title
    end if
    make_ deep + deep_start, v, result
    for each txt in result
      echo deep + deep_start, txt
    next
  end sub
...
'---
  ...
  private sub obj_(deep, tn, v, result)
    dim val
    if tn = "File" or tn = "Folder" then
      val = v
    else
      val = "(" & tn & ")"
    end if
    if mSHOW_TYPE then
      if tn = "File" or tn = "Folder" then
        result.add type_(deep, tn) & " " & val
      else
        result.add type_(deep, tn)
      end if
    else 
      result.add make_indent(deep) & val
    end if
  end sub
