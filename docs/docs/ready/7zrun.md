...
Позволяет:
  ...
  - задавать макс. размер файла: файлы больше указанного порога будут игнорироваться  
  - подгружать персональные настройки из файла `7zrun.cfg`  
...
7zrun.cfg
---------
Файл `7zrun.cfg` позволяет держать дополнительные настройки отдельно от `7zrun.bat`.  
Скрипт ищет файл рядом с собой, то есть в том же каталоге, где лежит `7zrun.bat`.  

Настройки применяются блоками. 
Каждый блок начинается со строки `eDIR_SRC=...`: она задает каталог, для которого предназначены следующие параметры.  
Блок считается подходящим, если текущий `eDIR_SRC` полностью совпадает с указанным путем или заканчивается на него как на отдельный каталог.  
Например, блок `eDIR_SRC=hybrids` подойдет для `D:\local\hybrids`, но не для `D:\local\my-hybrids`.  

Формат файла простой:  
  - пустые строки игнорируются  
  - строки, начинающиеся с `#`, считаются комментариями  
  - остальные строки имеют вид `имя=значение`  
  - имена параметров не зависят от регистра  
  - если в файле несколько подходящих блоков, применяются все по порядку; для обычных параметров побеждает последнее значение  
  - `eEXCLUDE` накапливается: все подходящие значения добавляются к маскам из окружения/батника  

Поддерживаемые параметры внутри блока:  
  - `eSTYLE`  
  - `eTIMESPAMP`  
  - `ePASSWORD`  
  - `eDIR_DST`  
...
```ini
# настройки для любого пути, который заканчивается на \hybrids
eDIR_SRC=hybrids
eSTYLE=root-ver\date-pre
eDIR_DST=D:\backup
eEXCLUDE=msvc-temporary;_backup
eEXCLUDE_SIZE=100mb

# отдельные настройки для конкретного проекта
eDIR_SRC=D:\local\example
eNAME7Z=example
eNOSTASH=ON
eEXCLUDE=bin;obj;*.tmp
```

`eDIR_SRC` из `7zrun.cfg` используется только как маркер начала блока и условие применимости.  
Сам каталог-источник по-прежнему определяется обычными правилами `7zrun.bat` и не переопределяется из конфига.  
<br/>
...
| eEXCLUDE_SIZE | файлы больше указанного размера игнорируются | выкл                                                 |
| eCLASSIC      | вкл классический режим                       | выкл. по умолчанию действует typical mode            |
| eNOSTASH      | вкл режим "без стеша"                        | выкл                                                 |

Размер для `eEXCLUDE_SIZE` можно задавать в байтах или с суффиксами `b`, `k`, `kb`, `m`, `mb`, `g`, `gb`.  
Например: `500000`, `512kb`, `100mb`, `2g`. Если значение пустое или не распознано как число, ограничение выключается.  
Файлы исключаются только если их размер строго больше указанного порога.  
...
|:------:|:---------------:|:----------:|:-----:|:---------------:|:-------:|:-------:|  
|  0003  | 0.0.1 [![P]][M] | 2024-12-08 | 19:00 | [#59-dev-7zrun] | VERSION |  0.0.1  |  
|  0002  | 0.0.1 [![P]][M] | 2024-12-08 | 18:50 | [#59-dev-7zrun] |  DONE   |  0.0.1  |  
